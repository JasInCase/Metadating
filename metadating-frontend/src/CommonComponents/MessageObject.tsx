...
const MessageObject = (props: MessageData) => {
	...
	return (
		...
		{is_user &&
			...
			<ListItem /*onMouseOver={() => handleMouseOver(index)} onMouseOut={handleMouseOut}*/ className={`${is_practice ? "bg-cyan-400" : "bg-blue-400"} animate-ping-once rounded-3xl`} style={{ display: 'flex', justifyContent: 'flex-end' }}>
		...
		{!is_user &&
			...
			<ListItem key={message} className={`${is_practice ? "bg-cyan-100" : "bg-slate-200"} rounded-3xl animate-ping-once`}>
				<ListItemAvatar className='m-2 hover:shadow-2xl'>
